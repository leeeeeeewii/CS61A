...
class ProfessorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        op_deck_copy = opponent.deck.cards[:]
        for j in range(len(op_deck_copy)-1,-1,-1):
            if op_deck_copy[j].attack == opponent_card.attack or op_deck_copy[j].defense == opponent_card.defense:
