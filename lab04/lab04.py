...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    coordinate = make_city('_', lat, lon)
    return get_name(city_a) if distance(coordinate, city_a) < distance(coordinate, city_b) else get_name(city_b)
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[i // 2 + len(deck) // 2] if i % 2 == 1 else deck[i // 2] for i in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    return tree(label(t1) + label(t2),
                [add_trees(x, y) for x, y in itertools.zip_longest(branches(t1), branches(t2), fillvalue=tree(0))])
