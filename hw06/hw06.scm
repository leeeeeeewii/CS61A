...
(define (pow base exp) (if (= exp 0) 1 (if (even? exp) (square (pow base (/ exp 2))) (* base (square (pow base (/ (- exp 1) 2)))))))
...
(define (without-duplicates lst)
    (if (null? lst) ()
    (cons (car lst) (without-duplicates (filter (lambda (x) (not (= (car lst) x))) (cdr lst))))
    )
)
...
(define (accumulate-tail merger start n term)
    (if (>= n 1)
        (accumulate-tail merger (merger start (term n)) (- n 1) term)
        start
    )
)
